...
int main()
{
    ...
    auto global_robot = boost::shared_ptr<hexapod_robdyn::Hexapod>(new hexapod_robdyn::Hexapod(*global_env, Eigen::Vector3d(0, 0, 0.5), brk));
    ...
    hexapod_robdyn::HexapodRobdynSimu simu(ctrl, global_robot);
    for (int i = 0; i < 167; i++)
        simu.run(0.015, true);
